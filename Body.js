    ...
    return (
        <div className="body-container">
            <button className="filter-btn" onClick={() =>{ 
               let FilteredList = ListOfRestaurants.filter(res => res.data.data.avgRating > 4);
               setListOfRestaurants(FilteredList);
            }}>Top Rated Restaurants</button>

            <button className="filter-btn" onClick={()=>{
                let FilteredList = ListOfRestaurants.filter(res => res.data.data.veg == true);
                setListOfRestaurants(FilteredList);
            }}> 100% Vegetarian Restaurants</button>
            
            
            <div className="res-container">

            {ListOfRestaurants.map((restaurant)=>{
               return(<RestaurantCard key={restaurant.data.data.id} resData = {restaurant}/>) 
